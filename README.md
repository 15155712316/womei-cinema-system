...
| 影院座位图UI     | 已完成   | AI     | YYYY-MM-DD   | 2024-05-XX   | [Canvas方案技术细节](#技术实现细节（座位图Canvas方案）) |
...
### 影院座位图UI（SeatMapPanel）
- **实现方式**：由原先的tk.Button方案升级为tk.Canvas方案。
- **每个座位为黑色边框的矩形**，可售为白色、已售为灰色、已选为绿色，视觉效果与真实影院座位图一致。
- **交互逻辑**：
  - 可售座位可点击，切换选中/取消状态。
  - 已售座位不可点击。
  - 选中状态实时刷新。
- **图例**：用Canvas绘制，风格与座位区一致。
- **接口兼容**：保留`update_seats`、`get_selected_seats`等方法，便于与其他模块对接。
- **代码已详细注释，便于维护和扩展。**

### 变更说明
- 2024-05-XX：将座位渲染方式从Button改为Canvas，彻底解决了Windows下按钮边框不可控的问题，实现了自定义黑色边框和更美观的座位图。
