...
class CinemaOrderSimulatorClassicWindow(QMainWindow):
    ...
    def _on_user_login_success(self, user_info):
        ...
        try:
            ...
            login_machine_code = user_info.get("machine_code", "")
            
            if current_machine_code != login_machine_code:
                QMessageBox.critical(
                    self, "登录失败", 
                    f"机器码验证失败\n当前机器码: {current_machine_code}\n登录机器码: {login_machine_code}"
                )
                self._restart_login()
                return
            
            # 3. 网络API验证（调用真实API验证账号状态）
    ...
    def _get_cinema_name_by_id(self, cinema_id: str) -> str:
        ...
        try:
            ...
            if os.path.exists(cinemas_file):
                ...
                for cinema in cinemas:
                    if cinema.get("cinemaid") == cinema_id or cinema.get("cinema_id") == cinema_id:
                        return cinema.get("cinemaShortName", cinema.get("name", "未知影院"))
    ...
    def _get_cinema_id_by_name(self, cinema_name: str) -> Optional[str]:
        ...
        try:
            ...
            if os.path.exists(cinemas_file):
                ...
                for cinema in cinemas:
                    if (cinema.get("cinemaShortName") == cinema_name or 
                        cinema.get("name") == cinema_name):
                        return cinema.get("cinemaid", cinema.get("cinema_id"))
