...
class CinemaOrderSimulatorClassicWindow(QMainWindow):
    ...
    def _on_cinema_account_login(self):
        ...
        try:
            ...
            if not current_cinema or current_cinema == "请先选择影院":
                QMessageBox.warning(self, "选择错误", "请先选择影院")
                return
            
            # 获取影院ID
            cinema_id = self._get_cinema_id_by_name(current_cinema)
            if not cinema_id:
                QMessageBox.warning(self, "获取失败", "获取影院ID失败")
                return
    ...
    def _cinema_account_login_api(self, phone: str, openid: str, token: str, cinema_id: str):
        ...
        try:
            ...
            QMessageBox.critical(self, "登录失败", f"登录API调用失败: {str(e)}")
    ...
    def _save_cinema_account(self, account_data: dict):
        ...
        try:
            ...
            QMessageBox.critical(self, "保存失败", f"保存账号失败: {str(e)}")
            print(f"[账号保存] 错误: {e}")
    ...
    def _on_account_selection_changed(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "账号选择错误", f"账号选择处理失败: {str(e)}")
    ...
    def _set_current_account(self, account_info: dict):
        ...
        try:
            ...
            QMessageBox.critical(self, "设置账号失败", f"设置当前账号失败: {str(e)}")
    ...
    def _on_account_changed(self, account_info: dict):
        ...
        try:
            ...
            QMessageBox.critical(self, "账号切换错误", f"账号切换处理失败: {str(e)}")
    ...
    def _on_cinema_changed(self, cinema_text: str):
        ...
        try:
            ...
            QMessageBox.critical(self, "影院选择错误", f"影院选择处理失败: {str(e)}")
    ...
    def _load_movies(self, cinema_id: str):
        ...
        try:
            ...
            QMessageBox.critical(self, "影片加载错误", f"加载影片失败: {str(e)}")
    ...
    def _on_movie_changed(self, movie_text: str):
        ...
        try:
            ...
            QMessageBox.critical(self, "影片选择错误", f"影片选择处理失败: {str(e)}")
    ...
    def _load_dates(self, cinema_id: str, movie_title: str):
        ...
        try:
            ...
            QMessageBox.critical(self, "日期加载错误", f"加载日期失败: {str(e)}")
    ...
    def _on_date_changed(self, date_text: str):
        ...
        try:
            ...
            QMessageBox.critical(self, "日期选择错误", f"日期选择处理失败: {str(e)}")
    ...
    def _load_sessions(self, cinema_id: str, movie_title: str, date: str):
        ...
        try:
            ...
            QMessageBox.critical(self, "场次加载错误", f"加载场次失败: {str(e)}")
    ...
    def _on_session_changed(self, session_text: str):
        ...
        try:
            ...
            QMessageBox.critical(self, "场次选择错误", f"场次选择处理失败: {str(e)}")
    ...
    def _load_seat_map(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "座位图加载错误", f"加载座位图失败: {str(e)}")
    ...
    def _load_coupons(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "券加载错误", f"加载券列表失败: {str(e)}")
    ...
    def _on_submit_order(self):
        ...
        try:
            ...
            if not self.current_cinema_id:
                QMessageBox.warning(self, "影院错误", "请先选择影院")
                return
            ...
            if not all([cinema_text, movie_text, date_text, session_text]) or \
               any(text in ["请先选择", "正在加载", "暂无", "加载失败"] for text in [cinema_text, movie_text, date_text, session_text]):
                QMessageBox.warning(self, "选择错误", "请完整选择影院、影片、日期和场次")
                return
            
            # 获取选择的座位
            selected_seats = self.seat_input.text().strip()
            if not selected_seats:
                QMessageBox.warning(self, "座位错误", "请输入选择的座位")
                return
    ...
    def _create_order_api(self, order_data: dict):
        ...
        try:
            ...
            if order_result.get("success"):
                ...
                QMessageBox.information(self, "提交成功", "订单提交成功！")
            else:
                QMessageBox.critical(self, "提交失败", f"订单创建失败: {order_result.get('message', '未知错误')}")
                
        except Exception as e:
            QMessageBox.critical(self, "创建订单错误", f"创建订单API调用失败: {str(e)}")
...
请在15分钟内完成支付"""
            ...
            QMessageBox.critical(self, "显示订单错误", f"显示订单详情失败: {str(e)}")
    ...
    def _start_countdown(self, expire_timestamp: int):
        ...
        try:
            ...
            QMessageBox.critical(self, "倒计时错误", f"启动倒计时失败: {str(e)}")
    ...
    def _update_countdown(self, expire_timestamp: int):
        ...
        try:
            ...
            if remaining_seconds <= 0:
                ...
                QMessageBox.warning(self, "订单过期", "订单已过期，请重新下单")
            ...
            QMessageBox.critical(self, "倒计时错误", f"更新倒计时失败: {str(e)}")
    
    def _on_one_click_pay(self):
        """一键支付处理"""
        try:
            if not self.current_order:
                QMessageBox.warning(self, "订单错误", "没有待支付的订单")
                return
            ...
            QMessageBox.critical(self, "支付错误", f"支付处理失败: {str(e)}")
    ...
    def _process_payment(self):
        ...
        try:
            ...
            if payment_result.get("success"):
                ...
                QMessageBox.information(self, "支付成功", "支付成功！请凭取票码到影院取票")
                
                # 清空当前订单
                self.current_order = None
                
            else:
                QMessageBox.critical(self, "支付失败", f"支付失败: {payment_result.get('message', '未知错误')}")
                
        except Exception as e:
            QMessageBox.critical(self, "支付错误", f"支付流程处理失败: {str(e)}")
