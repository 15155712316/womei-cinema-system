...
class CinemaOrderSimulatorClassicWindow(QMainWindow):
    ...
    def _on_user_login_success(self, user_info):
        ...
        try:
            ...
            self.message_manager.show_info(f"欢迎使用柴犬影院系统，{user_info.get('username', '')}")
    ...
    def _save_cinema_account(self, account_data: dict):
        ...
        try:
            ...
            for i, acc in enumerate(accounts):
                if acc.get("phone") == account_data["phone"] and acc.get("cinema_id") == account_data["cinema_id"]:
                    # 更新现有账号
    ...
    def _get_cinema_id_by_name(self, cinema_name: str) -> Optional[str]:
        ...
        try:
            ...
            # 临时实现：返回示例ID
            cinema_mapping = {
                "华夏优加荟大都荟": "huaxia_daduhui",
                "万达影城": "wanda_cinema", 
                "CGV影城": "cgv_cinema"
            }
            return cinema_mapping.get(cinema_name)
        except Exception as e:
            print(f"获取影院ID失败: {e}")
            return None
    ...
    def _refresh_account_list(self):
        ...
        try:
            ...
            for i, account in enumerate(accounts):
                self.account_table.setItem(i, 0, QTableWidgetItem(account.get("phone", "")))
                self.account_table.setItem(i, 1, QTableWidgetItem(account.get("cinema_name", "")))
                self.account_table.setItem(i, 2, QTableWidgetItem(str(account.get("balance", "0"))))
    ...
    def _refresh_cinema_list(self):
        ...
        try:
            ...
            self.cinema_combo.addItems([cinema.get("name", "") for cinema in cinemas])
    ...
    def _on_account_selection_changed(self):
        ...
        try:
            ...
            if current_row >= 0:
                phone = self.account_table.item(current_row, 0).text()
    ...
    def _set_current_account(self, account_info: dict):
        ...
        try:
            ...
            # 更新当前账号显示
            current_account_text = f"当前账号: {account_info.get('phone', '')} (余额:{account_info.get('balance', '0')})"
            
            # 这里需要找到当前账号显示的QLabel并更新
            # 由于UI结构，我们需要保存这个标签的引用
